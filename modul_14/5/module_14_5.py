...
api = "7757008094:AAFGBm8NnqZllHNgPp5QHz5uJHbhAMSWbl0"
bot = Bot(token=api)
...
async def get_buying_list(message):
    for i in all_products:
        way = f'../scrins/Screenshot_{i[0]}.jpg'
        with open(way, "rb") as img:
...
async def send_calories(message, state):
    ...
    await message.answer(f'Ваша норма калорий для сохранения нормального веса или похудения составляет:\n'
                         f' {(10 * int(data["weight"])) + (6.25 * int(data["growth"])) - (5 * int(data["age"])) + 5} ккал в сутки')
    await state.finish()
...
async def set_username(message, state):
    user = message.text
    if crud_functions.is_included(user):
